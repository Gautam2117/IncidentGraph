...
export default function Sidebar() {
  ...
  return (
    ...
    </aside><nav aria-label="Mobile console navigation" className="fixed inset-x-3 bottom-3 z-50 grid grid-cols-4 rounded-2xl border border-white/10 bg-[#080c12]/95 p-1.5 shadow-2xl backdrop-blur lg:hidden">{mobileItems.map((item) => { const active = pathname === item.href || pathname.startsWith(`${item.href}/`); return <Link key={item.href} href={item.href} className={`rounded-xl px-1 py-2.5 text-center text-[10px] font-medium transition ${active ? 'bg-cyan-400/10 text-cyan-200' : 'text-slate-500 hover:text-white'}`}>{item.label}</Link>; })}</nav></>
  );
}
